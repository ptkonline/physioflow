...
const DoctorCaseloadChart = dynamic(
  () => import("@/components/doctor/DoctorCaseloadChart").then((mod) => mod.DoctorCaseloadChart),
  { loading: () => <p className="text-muted">Loading caseload chart…</p>, ssr: false },
);
...
export default function DoctorDashboardPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      <DoctorCaseloadChart physioId={user.id} />
          ...
          {patients.map((p) => {
            ...
            return (
              <li key={p.userId} className="flex flex-wrap items-center justify-between gap-3 py-3">
                ...
                <Link href={`/doctor/patients/${p.userId}`} className="btn btn-primary">
                  Open record
                </Link>
              </li>
            );
